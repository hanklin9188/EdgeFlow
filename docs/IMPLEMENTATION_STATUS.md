...
> 最新的逐項進度與待辦清單請以
> [CURRENT_PROGRESS.md](CURRENT_PROGRESS.md) 為準；本文保留較細的 implementation evidence。

更新日期：2026-08-09。此表把「工程能力完成」與「需要長時間正式實驗才能成立的研究結論」分開，避免以程式存在冒充實驗完成。
...
| M3 Select by Workload | candidate pruning、session objective、policy/fallback/drift、E20 fresh-process cold/warm runner；E10 最大完整 plan 子矩形分析 | E06 正式 mixed-shape pass；E20 cached-host 30-pair scope pass；E10 Plan A 45/45、Plan B 23/45，common grid 23/45，仍 incomplete；E19 exact-scope break-even `PASS` | 不允許把 23/45 partial common grid 外推成完整雙 plan policy 結論 |
...
| M6 Learned component | E25/E27 unique-point 與 intervention-label exporter、prerequisite audit 已完成 | 目前 80/2,000 unique validated points、0/50 per-class intervention labels | 門檻前拒絕訓練，不以 repetitions 或重跑灌水 |
...
E00–E30 是實驗 catalog，不是可由 unit test 取代的 checkbox。primary Ministral 3B 的 llama.cpp Q5_K_M 與 vLLM BF16 quality、正式 `p1024-o128-c1` runs 與 E09 exact-scope audit 已成立；Q4_K_M 因 ARC-C drop 超過 1pp 保留為 negative evidence。E10 graph-mbt32768 profile 已 45/45 `PASS`，graph-mbt65536 profile 現有 23/45，兩 plan common grid 為 23/45，因此整體仍是 `INCOMPLETE`。E11–E18 仍需要 real-distribution replay、controlled mediator、negative-control 與 holdout pairs。E21 profile-first candidate evidence 已 `PASS`。E25 目前有 80/2,000 unique validated points，E26 因 prerequisite 未達而拒絕訓練；E27 尚無每類 50 筆正式 intervention labels。E28 固定 22 題 grounding test 已 `PASS`。E29 還需要至少 5 位同領域使用者完成 usability tasks；E30 需要另一台機器或真正乾淨環境重現。只有通過 G0–G8 的數字才能進 README。
