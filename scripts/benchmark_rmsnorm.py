...
from edgeflow.kernels.rmsnorm.dispatch import (  # noqa: E402
    MIN_PRACTICAL_SPEEDUP,
    record_performance_decisions,
    validate_shape,
)
...
def main() -> int:
    ...
    if not torch.cuda.is_available():
        ...
        return 2
    shapes = (
        [(1, 768), (8, 1000), (32, 4096)]
        if arguments.quick
        else [
            (row, hidden)
            for row in (1, 2, 4, 8, 16, 32, 128, 512)
    ...
    iterations = 100
    rows = []
    combinations = [(dtype, *shape) for dtype in dtypes for shape in shapes]
    random.Random(42).shuffle(combinations)
    gpu_name = torch.cuda.get_device_name(0)
    for dtype, row_count, hidden in combinations:
        ...
        functions = {
            ...
            "torch_compile": lambda x=x, residual=residual, weight=weight,
            compiled=compiled_reference: compiled(x, residual, weight),
        }
    ...
    dispatch_cache = record_performance_decisions(rows)
    faster_rows = sum(row["speedup"] >= MIN_PRACTICAL_SPEEDUP for row in rows)
    payload = {
        ...
        "protocol": {
            ...
            "iterations": iterations,
            "timer": "cuda_event",
            "shape_order": "seeded_random",
            "compile_scope": "fresh_static_fullgraph_per_shape",
        },
        "rows": rows, "all_correct": all(row["correctness"]["status"] == "PASS" for row in rows),
        "dispatch_rule": {
            "baseline": "pytorch_eager_reference",
            "minimum_practical_speedup": MIN_PRACTICAL_SPEEDUP,
            "triton_enabled_rows": faster_rows,
            "fallback_rows": len(rows) - faster_rows,
            "cache_entries": len(dispatch_cache),
            "unmeasured_shape_behavior": "reference_fallback",
        },
