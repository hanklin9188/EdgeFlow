...
- `evaluate_llama_cpp_quality.py` — compares pinned BF16 GGUF and a registered quantization with llama-perplexity and the same fixed WikiText/ARC selection; a failed quantization remains negative evidence.
...
- `run_external_bucket.py` — runs one exact-token llama.cpp or vLLM bucket after matching runtime/format/quantization quality evidence exists.
- `run_e10_grid.py` — resumes either preregistered 45-bucket Ministral vLLM scheduler grid and never counts an ineligible prior case as complete.
...
- `audit_learned_layer.py` — exports deduplicated E25/E27 snapshots and keeps E25–E27 blocked until the registered 2,000-point and 50-label gates are met.
- `run_grounding_test.py` — executes the fixed E28 numeric-fidelity, citation, refusal, and unsupported-claim gates.
- `run_e21_profile_selection.py` — records a literal CUDA profile before ranking the next custom-kernel candidate; it does not rewrite the chronology of the earlier RMSNorm result.
